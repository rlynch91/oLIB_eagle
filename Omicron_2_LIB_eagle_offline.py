...
def calculate_livetimes(seg_files1, seg_files2, ts_list_1, ts_list_2, ifos1, ifo2, coin_group, coin_mode, ppdir):
	...
	if not os.path.exists("%s/livetime/%s/%s/"%(ppdir,coin_group,coin_mode)):
		os.makedirs("%s/livetime/%s/%s/"%(ppdir,coin_group,coin_mode))
	if (coin_mode == "0lag") or (coin_mode == "sig_train"):
		np.savetxt("%s/livetime/%s/%s/livetime_0lag_%s%s.txt"%(ppdir,coin_group,coin_mode,ifos1,ifo2), np.array([zero_lag_lt]))
	elif (coin_mode == "back") or (coin_mode == "noise_train"):
		np.savetxt("%s/livetime/%s/%s/livetime_timeslides_%s%s.txt"%(ppdir,coin_group,coin_mode,ifos1,ifo2), np.array([timeslide_lt]))
...
if __name__=='__main__':
	...
	parser.add_option("-o", "--outdir", default=None, type="string", help="Path to directory in which to save results")
	parser.add_option("-t", "--trigdir", default=None, type="string", help="Path to directory containing raw omicron triggers")
	...
	if run_mode == 'Offline':
		...
		if not sig_train_flag:
			ppdir = "%s/PostProc/"%outdir
			rawdir = trigdir
		else:
			ppdir = "%s/PostProc_sig_train/"%outdir
			rawdir = trigdir
