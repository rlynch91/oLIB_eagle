...
if __name__=='__main__':
	...
	if run_dic['run mode']['email flag']:
		...
		if os.path.isfile(run_dic['config']['run dir']+'/last_failure_email.txt'):
			last_failure_email = int(np.genfromtxt(run_dic['config']['run dir']+'/last_failure_email.txt'))
		else:
			last_failure_email = -np.inf
		...
		if current_gps_time >= (last_failure_email + run_dic['run mode']['email throttle']):
			...
			#Then mark send time
			os.system('echo %s > %s/last_failure_email.txt'%(current_gps_time,run_dic['config']['run dir']))
