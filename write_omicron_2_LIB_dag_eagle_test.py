	...
	if run_dic['run mode']['line'] == 'Online':
		...
		if train_runmode == 'Train':
			...
			for ifo in ifos:
				...
				#replace all necessary variables in params file
				os.system('sed -e "s|IFO|%s|g" -e "s|FRAMECACHE|%s|g" -e "s|CHNAME|%s|g" -e "s|SAMPFREQ|%s|g" -e "s|OLAP|%s|g" -e "s|STRIDE|%s|g" -e "s|RAWDIR|%s|g" -e "s|MINFREQ|%s|g" -e "s|MAXFREQ|%s|g" -e "s|THRESHSNR|%s|g" -e "s|//INJECTION|INJECTION|g" -e "s|INJCACHE|%s|g" %s/omicron_params_eagle.txt > %s/runfiles/omicron_sig_train_params_%s_eagle.txt'%(ifo, cache_files[ifo], channel_names[ifo], sample_freq, overlap, stride, segdir+'/raw_sig_train/'+ifo, min_freq, max_freq, osnr_thresh, sig_train_cache_files[ifo], infodir, segdir, ifo))
